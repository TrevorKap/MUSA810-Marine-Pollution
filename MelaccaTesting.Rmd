...
```{r}
# load litter data
litter <- read.csv('https://raw.githubusercontent.com/TrevorKap/MUSA810-Marine-Pollution/main/Data/Melacca-data.csv')
...
mumb_bdry <- st_read('https://github.com/TrevorKap/MUSA810-Marine-Pollution/raw/main/Data/Melacca.kml')
...
water<- c('canal','drain','ditch', 'harbor', 'wastewaster') # done 
leisure <- c('park')
...
india <- getbb(place_name = "Melacca", format_out = "polygon") # not used 
bb_df <- getbb(place_name = "Melacca, Malaysia", format_out = "data.frame") # get chennai boundary
bdry <- bb_df$boundingbox # extract the geoemtry part
bdry
mumb_bd <- c(102.3916889, 2.3724081, 102.1911847, 2.1718442) # just a record
# 4, 2, 3, 1
```

```{r get point_nn_fishnet}
water_point <- point_data(mumb_bd, 'water',c('canal','drain','ditch', 'harbor', 'wastewaster'),'waterway') # grab osm point data (the detail input variable just check the 'https://raw.githubusercontent.com/TrevorKap/MUSA810-Marine-Pollution/main/function_UO.R' and I have write the input require)
water_net <- countfishnet(fishnet,water_point) # create fn variable of litter count (for panama didnt work)
...
```{r}
#print(chisq.test(water_nn_net$count,litter_net$count)) # chi-square test 
...
#visual_count_net(water_nn_net,water_point,mumb_bdry,'water') # visualize the point and fn count data
...
local_morans_road <- localmoran(final_net$road, final_net.weights, zero.policy=TRUE) %>% 
  as.data.frame()
local_morans_wtr <- localmoran(final_net$watr_nn, final_net.weights, zero.policy=TRUE) %>% 
  as.data.frame()
```
...
```{r}
reg.ss.vars <- c("restaurant_nn", "rst_sig_dis","road_nn", "road_sig_dis") # the independent variables included
...
risk_v(reg.ss.spatialCV,litter_p,'kmeans')
```
