...
mumb_bdry <- st_read('https://github.com/TrevorKap/MUSA810-Marine-Pollution/raw/main/Data/Santiago.kml')
...
india <- getbb(place_name = "Chile", format_out = "polygon") # not used 
bb_df <- getbb(place_name = "Santiago, Chile", format_out = "data.frame") # get chennai boundary
bdry <- bb_df$boundingbox # extract the geoemtry part
bdry
mumb_bd <- c(-70.6249963, -33.4255933, -70.6920527, -33.4785698) # just a record
# 4, 2, 3, 1
```

```{r get point_nn_fishnet}
water_point <- point_data(mumb_bd, 'water',c('canal','drain','ditch', 'harbor', 'wastewaster'),'water') # grab osm point data (the detail input variable just check the 'https://raw.githubusercontent.com/TrevorKap/MUSA810-Marine-Pollution/main/function_UO.R' and I have write the input require)
...
waste_nn_net <- knnfishnet(fishnet,waste_point,3,'waste')
