...
## Concluded Results
 ...
 Trevor: redo paragraph below
 Stephanie: Spell check (starting at top)
 
Talk about inverse in p value hot spot in rest sig maps

from the chi.sq test for the association test, the restaurant (represent the human activity ), the water (no obvious causation between two variables), and the residential road density(I use the count in each fishnet unit to represent density) have some association with litter distribution. and the waste management seems has little association. Chi-square test it’s a good approach to analysis the two categorical variables’ association, but in this case, the road seems to have little association but still proved strongly associated, if there is another way to check the more associated variables? perhaps local Moran’s I to decrease the influence of data lack? or other methods? The next step, if we get several more strongly associated variables (I mean, test around some other cities in India and still proved associated), if our target get converted to ‘use this variables to build a weight model to assess the risk for generating litter’? whether it’s necessary to use ML? or ML model is functioning more like a testing tool?
...
This model requires us to re-examine similar or duplicate variables and measuring their integrity for acting as indicators of litter. 


### Nearest Neighbor Series 

Here, we further examine the nearest neighbor distances of restaurants, roads, and water, all of which have identical patterns to previous analyses. 
...
acting as a predictor in addition to displaying spatial autocorrelation
...
After a full analysis on variables, model development, and statistical evaluation of all relations, we developed a risk assessment model. The results that follow examine the numeric, visual, and decision implications of the model's outcome. 

## Mean Absolute Error of Model

We see negligible error for vast majority of litter. 
...
Assuming litter found in one spot means litter found in its surrounding, we created random points in the surrounding area to act as additionality to data quantity. 
...
We examine the accuracy and integrity of the model before looking at the results. 

#### Bias Reduction Model, Mean Absolute Error

We see slightly more erroror compared to before. 
...
We see similar results as from the first model further proving the integrity of the data evaluated prior. 
