...
On this fishnet grid we notice sections adjacent to the coastline (east) create two centers with low 'nearest neighbor' values, also places that loosely defined outlined sections simlar to restaurants.  
...
Here we can make two assumptions regarding the influence of waste management and its effects litter distribution: 
...
Restaurants are signficant because xyz 
...
### the period result (whats period result)
...
Here we developed a temporary model to
...
We wanna show relations by having them side by side 
...
1-5 are the categories
the encirlced data, why it says 5th category without any actual litter there. 
...
Assuming litter found in one spot means litter found in its surrounding, creates random points in the surrounding area and goes off that. 
...
lowkey same stuff so either we didnt do anything about bias or the model already compensated
...
we are doing random forst now because why not 
