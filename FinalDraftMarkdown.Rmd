...
plotTheme <- function(base_size = 12, title_size = 16) {
  theme(
    ...
    plot.title = element_text(size = title_size, family = "Open Sans", colour = "black"), 
    plot.subtitle = element_text(face = "italic", family = "Open Sans"),
    plot.caption = element_text(hjust = 0, family = "Open Sans"),
    ...
    strip.text = element_text(size = 12, family = "Open Sans"),
    axis.title = element_text(size = 12, family = "Open Sans"),
    axis.text = element_text(size = 10, family = "Open Sans"),
    plot.background = element_blank(),
    legend.background = element_blank(),
    legend.title = element_text(colour = "black", face = "italic", family = "Open Sans"),
    legend.text = element_text(colour = "black", face = "italic", family = "Open Sans"),
    strip.text.x = element_text(size = 14, family = "Open Sans")
  )
}
...
mapTheme <- function(base_size = 12, title_size = 16) {
  theme(
    text = element_text(family = "Open Sans", color = "black"),
    plot.title = element_text(size = title_size, colour = "black", family = "Open Sans"),
    plot.subtitle = element_text(face = "italic", family = "Open Sans"),
    plot.caption = element_text(hjust = 0, family = "Open Sans"),
    axis.ticks = element_blank(),
    panel.background = element_blank(),
...
Ocean Conservancy has taken the initiative to organize a global network of cities to combat marine pollution through the Urban Ocean program. This program takes multiple approaches to address ocean plastic pollution. First, they mobilizes coastal cleanup campaigns across each city based on volunteer efforts with international and local support groups, such as the International Coastal Cleanup. Second, they research each system's sanitation and urban systems to identify key details regarding what causes trash to end up  in oceans & waterways, sharing findings and recommended action plans to local communities and decision makers. And third, working with stakeholders directly to create changes in products, practices, and behaviors as alternative solutions to their current needs. The program has been successful in engaging local communities, businesses, and governments to take action against marine pollution.

With these efforts at the local level, each campaign requires an evaluation of local context and detailed understanding of the city's systems and dynamic. This aspect of the process requires long-term planning and methodical decision-making to understand the pre-existing systems in place. Hence, our model aims to streamline the process of understanding the general structure of each context and provide a comprehensive solution on where efforts and resources should be allocated to. Ultimately saving time on where to implement their waste-reduction strategies. 

We based our focus on various cities from the Resilient Cities Network and Urban Ocean program. The Resilient Cities Network is a coalition of cities around the world that assist one another in building safer, more sustainable, and equitable futures for their populations. Each of which shares their knowledge, experiences, and policy practices to empower one another. A subsection of the Resilient Cities Network is the Urban Ocean program, a partnership with Ocean Conservancy tackling the ocean plastic problem. The cities displayed as examples for our model are Chennai India; Bangkok, Thailand; and Santiago, Chile. The three cities mentioned are apart of the Urban Ocean 12 city networking with the goal of proactively enhancing their sanitation infrastructure to combat plastic pollution in their region. 

The 12 participating cities apart of the Urban Ocean program as of May 2024 are:
...
map_waste_nn <- visual_cotinuous(net_chennai,'waste_nn')
...
grid.arrange(map_restaurant, map_road, map_industrial,
...
The data regarding each region and its attributes was sourced from OpenStreetMap (OSM) to ensure a repeatable framework & quality consistency.

What we used in OSM data
...
df_model <- st_drop_geometry(net_)%>%dplyr::select(!cvID)
...
### 3 variables trees
```{r 3 variable trees, warning = FALSE, message = FALSE, cache = TRUE }
...
```{r rst total, warning = FALSE, message = FALSE, cache = TRUE }
rst_total <- rst_total %>%
...
```{r rst total 4, warning = FALSE, message = FALSE, cache = TRUE }
